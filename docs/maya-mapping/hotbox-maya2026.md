...
日期：2026-09-09。状态：设计清单，未实现新版热盒；用户已确认总体结构，书面规格待审。
关联：[Phase 2B 设计](../superpowers/specs/2026-09-09-phase-2b-maya-hotbox-design.md)。

“2B 接入”是目标，不是完成标记。下文未特别列出的叶命令不自动进入 2B。
...
| 中央 | Hotbox Controls | 行显隐、透明度、三种显示样式 | Center Zone RMB Popups 功能组弹出模式另批接入，不改变默认视图右键 |
...
默认未自定义的中央 LMB/MMB/RMB 都能调用视图菜单；用户逐键覆盖。
