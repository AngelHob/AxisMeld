...
2026-09-11 用户明确：常用建模热盒优先，随后全面整理建模快捷键和菜单；UV 整体后置，
包括其交互层。功能按“原生可对应 / Maya 缺失能力占位并立项 / Blender 独有能力归入
对应功能目录”处理。当前范围与验收规则见
[建模交互优先对齐规格](../superpowers/specs/2026-09-11-modeling-interaction-alignment-design.md)。
下文保留历史阶段记录；其未实现列表不应被误读为禁止接入 Blender 相似能力。
