...
更新：2026-09-17。人工测试继续暂缓。历史253项和稳定编号保留，S/P/C/V/T/R共47项保持用户确认已测试，未补造通过/失败结果。本轮追加MB-01至MB-20，验证真正的顶部Menu Bar、五菜单集、Maya灰占位与Blender能力归类。本次追加MB-21至MB-27，验证章节归属、内置工作区及语义纠偏。MB-28/29双排方案已被用户后续要求替代，保留历史记录。本次追加MB-30至MB-34，验证Modeling视窗菜单和原生全局栏。本次追加MB-35至MB-39，验证建模分类、原生能力融合和右侧齿轮。本次追加MB-40至MB-41，验证紧凑菜单和公开试用反馈。本次追加RG-01至RG-06，验证Rigify默认内置、实际生成、文件重开、旧偏好保留、原生设置与重载。本次追加RG-07至RG-14，验证Rigging独立工作区、原生入口与Maya占位、绑定权重姿态、链接只读骨架数据、扩展入口和文件兼容。本次追加RG-15至RG-20，验证Skin归一化与小权重整理及独立Options。现合计314项：历史已测试47项，初始未测267项（其中12项为已替代的历史方案，不再作为当前验收要求）。自动化不覆盖用户手工结果。
...
用户确认 Rigify 主体默认内置，无需安装或启用附加组件。以下仅用本轮 Rigify 预览构建验证，不用旧 Modeling 预览包判断功能缺失。计划发布标签为 `axismeld-2026.09.15-rigify-preview`，当前发布状态以 GitHub Release 为准；编写清单不代表该构建已经发布或人工验收通过。测试准备与人工边界见 `../development/2026-09-15-rigify-manual-testing.md`。旧偏好、Feature Set 目录和场景均使用测试副本；恢复工厂只在独立临时配置进行。

| 编号 | 前置条件与操作 | 预期 | 人工状态 |
|---|---|---|---|
| RG-01 | 使用Rigify预览构建，分别以隔离工厂配置和普通方式启动；不安装、不手动启用Rigify，直接查看视窗Add→Armature中的metarig与骨架数据的Rigify入口 | 两种启动均默认提供真实Rigify菜单、属性和生成器；无需访问插件安装或启用界面，原生Armature入口仍可用；不自动安装或启用第三方Feature Set | 待测 |
| RG-02 | 在Rigify预览构建的临时场景，经视窗Add→Armature添加Human metarig；选中它，在骨架数据的Rigify面板执行Generate Rig；选择生成rig进入Pose Mode并移动手或脚控制骨 | 新建真实metarig并生成独立目标骨架、控制骨及控件；手脚约束随控制骨联动，原metarig保留，目标关联正确；没有只显示成功提示却未生成结果，也没有重复生成的意外副本 | 待测 |
| RG-03 | 将RG-02自建场景另存为临时blend，关闭并用同一Rigify构建重开；检查控制骨、metarig的Rigify属性和目标关联，再修改metarig并执行Re-Generate Rig | 保存的骨架、控件与Rigify数据仍在，重开不要求重新安装或启用；重生成按原目标更新且结果可操控；自建文件的生成脚本和驱动沿用Blender原有信任机制，不擅自降低全局安全设置 | 待测 |
| RG-04 | 准备旧Rigify偏好及Feature Set目录的临时副本，包含至少一个仍已安装但禁用的兼容Feature Set；记录其状态，用Rigify预览构建普通启动，保存临时偏好后再次启动 | Rigify主体仍默认可用；已有偏好及已安装Feature Set的名称、设置和启用状态保留，原禁用项不被自动启用或删除；其他用户偏好不被重置；缺少可用旧样本时保持待测并注明 | 待测 |
| RG-05 | 在Rigify预览构建打开Preferences→Animation→Rigify，检查Feature Sets列表与选中项详情；关闭并重新打开偏好，再与RG-04记录的设置对照 | 原生Animation设置中只有一份Rigify入口，直接展示真实Feature Set列表、状态和原有设置操作；无需先在Add-ons启用Rigify，原禁用项仍禁用，未安装推荐项不冒充已安装功能 | 待测 |
| RG-06 | 在独立临时配置和已保存的临时场景中，通过命令搜索执行Reload Scripts两次，每次检查Add→Armature和Rigify面板并创建、生成一个测试metarig；另用独立临时配置执行Load Factory Settings后重复 | 重载及工厂恢复后Rigify仍默认可用；菜单、面板和每次生成结果不重复，RNA属性与操作正常，无重复注册、缺少属性或遗留处理器错误；工厂重置仅作用于测试副本，不用于判断普通启动的旧偏好保留 | 待测 |

## P0：Rigging 独立工作区（新增8项）

以下项目使用本轮Rigging工作区预览构建，计划发布标签为 `axismeld-2026.09.17-rigging-preview`；实际发布以GitHub Release为准，不用旧Rigify预览包验收新增工作区。工厂配置、旧文件、绑定与权重操作均使用隔离测试副本。原有RG-01至RG-06内容和状态保留，新项目全部待人工测试；自动探针不能替代人工结果。测试边界见 `../development/2026-09-17-rigging-manual-testing.md`。
...
## P0：Skin 归一化与小权重整理（新增6项）

以下 RG-15 至 RG-20 使用 `axismeld-2026.09.17-skin-weights-preview` 或对应更新源码构建，实际下载以 GitHub Release 为准。旧 Rigging 包没有这两个 Maya 位置的适配。全部使用临时角色副本；自动测试不修改人工状态。旧 RG-01 至 RG-14 及其结果保持。完整规则见 `../development/2026-09-17-skin-weight-cleanup.md`。

| 编号 | 前置条件与操作 | 预期 | 人工状态 |
|---|---|---|---|
| RG-15 | 活动网格使用唯一 Armature 修改器，准备两变形骨组、锁定骨组、非变形骨组及普通遮罩组；从Skin→Normalize Weights→Normalize Weights执行并Undo，另检查全零顶点 | 只一次归一化变形骨权重，锁定值与非骨骼用途组保持；全零不自动赋权；一次Undo恢复。Disable/Enable/Post仍灰显，不把单组Normalize或持续模式当本功能 | 待测 |
| RG-16 | 同一临时网格从Skin→Prune Small Weights齿轮设置阈值，准备低于、等于、高于阈值及锁定小权重，测试Keep Strongest和Normalize After两选项并Undo | 只移除严格低于阈值的未锁定骨骼成员，等于阈值与锁值保留；默认保留最后最强正骨骼影响并归一化；明确关闭保护可清空；非骨骼组不影响最强保护，一次Undo恢复 | 待测 |
| RG-17 | 分别在Object、单对象Edit Mesh、Weight Paint无遮罩/顶点遮罩/面遮罩执行两项；加入隐藏和未选顶点、第二所选对象以及X镜像设置 | Object只处理活动网格未隐藏顶点；Edit只处理所选未隐藏顶点；绘制按当前遮罩；不扩展到镜像未选点、隐藏点或第二对象，不改选择或模式 | 待测 |
| RG-18 | 在常用缩放下分别点击两项正文和右对齐齿轮，打开参数后修改并取消，再确认执行；使用F9调整，再一次Undo | 正文用明确默认参数只执行一次；齿轮只打开参数，取消完全无修改；确认及F9作用于当前操作，Undo回到操作前；Normalize的临时Lock Active不改变永久组锁 | 待测 |
| RG-19 | 在无骨架、多Armature修改器、共享或链接Mesh、多对象Edit、空顶点选择和锁定和超过1的测试副本尝试两项；另用本地网格引用链接只读骨架 | 不支持范围安全灰显或整批取消，失败不留下部分权重修改、不暗改选择或模式；本地网格只读引用链接骨架可用，不能误禁用；没有给全零顶点猜测新影响 | 待测 |
| RG-20 | 在自己的生产角色副本执行有界清理后移动骨骼观察变形；保存到临时文件并用新进程重开，对照权重、组锁及原生Weights/插件菜单 | 保存重开数值、组成员和锁定状态一致，原生菜单与插件回调保留；记录角色规模、操作耗时和变形质量，未覆盖的第三方骨架或复杂场景不自动判通过 | 待测 |
