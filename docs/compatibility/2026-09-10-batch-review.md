...
集中手测入口见 [2026-09-13 待人工测试清单](2026-09-13-manual-test-ledger.md)。最新安装证据见[M2b目标拾取](2026-09-13-m2b-pointer-target.md)。此前布局修复证据见
[2026-09-12 热盒修正](2026-09-12-marking-hit-reference.md)，前批组件入口证据见
[M2a 组件右键验收](2026-09-12-m2a-component-hotbox.md)；以下记录为历史批次，哈希和旧等宽/父级显示描述不代表后续版本。
...
`C15AB0B4F8EB464901547B825974F69736B20D8A7D51CC08B1EE054E42D38ED6`。

回退前亮版实测0.537，不满足原主题0.4的回归断言；回退后安装态采样0.400、灰字0.624、
黑字0.043。安装态分层alpha专项通过，覆盖Transparency=0/25/75/100及二级原生悬停/分隔线。
41项Python、5组CTest、构建与静态复核通过。日志前缀`D:/source/AxisMeld-build/h5-primary-dark-`。
确认无用户进程后仅替换原入口exe，与构建SHA一致；6份portable配置及回退exe未变。
没有新增完整安装目录、关闭用户进程或推送。此前0.14亮版已被本版取代，记录不作为当前交付。

## 2026-09-11 一级底色提亮及75%不透明（历史版本，提亮已取消）
...
旧版默认/重置测试先失败，旧版真实GUI底色采样0.400失败；修正后约0.475，黑字0.047、灰字0.624。
3组真实GUI（底色与文字、native-style、分层alpha）及release GUI、profiles集成全部通过。
分层alpha覆盖Transparency=0/25/75/100，分别对比全透明/全不透明截图确认混合比例，
并验证二级、原生菜单入口和深层菜单不受一级或主题alpha影响。静态复核通过。
日志前缀`D:/source/AxisMeld-build/h5-primary-bright-`，`opacity.log`为标准native-style测试，
实际分层透明度专项为`alpha.log`。测试复用既有blender-opacity-check.exe及安装运行库，
...
检测旧窗口退出后复制了新exe及上轮待安装的两个Python模块，未关闭用户进程、未重置个人配置。

- 一级正常入口文字默认中灰#A0A0A0，未实现占位文字不透明黑色；背景和图标不改，悬停保持主题文字高亮。
- 因当前上下文不可用的真实入口不是未实现占位，仍保留禁用视觉；二级及更深不受本次文字规则影响。
- 已包含上轮原生不透明二级菜单、一级默认75%透明度、等宽椭圆和原生图标/分隔线修正。
- 当前个人Transparency本来就是75%，已保留；无需删除或迁移个人设置。

一级文字测试先在旧程序失败，修正后实际GUI通过：`h5-primary-text-red.log` / `h5-primary-text-green.log`。
测试浅色背景下的占位字与正常字最暗采样约0.075/0.624（含字体抗锯齿），对应黑字与灰字，
不是将占位按钮底色改黑。构建日志`h5-primary-text-build.log`，独立静态复核通过。
最终安装态验证（11:44）：41项Python测试、5组CTest通过；7组实际GUI通过（文字、native-style、
release、opacity、drag、quad、guide），另有profiles配置集成通过。日志前缀`h5-primary-final-`，
文字专项为`h5-primary-text-green.log`。exe与build、两个安装模块与源码逐项同SHA，
6份portable配置未变，回退exe仍为`F84108D2…DE02F`，测试进程已退出。没有推送或合并。

## 2026-09-11 默认样式与分层透明度（上轮构建记录，现已合入安装）
