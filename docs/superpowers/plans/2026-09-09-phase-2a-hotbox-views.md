...
8. Extend GUI event fixture for tap/hold/fast LMB cardinal gestures/dead zone/cancel/autorepeat/trigger remap through a temporary isolated test keymap. Assert view changes and region counts, not only operator calls. Clear temporary test keymaps before exit. These tests must run before production keymap connection, so explicitly install the fixture's native bindings. Failed commands never become layout taps.
9. Build only `blender` and changed test targets, config Release parallel 8, with log in build directory. Run native state test and new GUI test, then seven existing AxisMeld regressions once. Do not install until all tests pass and Task 3 connects the product.
...
**Files:** update `scripts/modules/axismeld_input/commands.py`, `adapter.py`, `profiles.py`, `keymap.py`, `runtime.py` only where necessary; update `scripts/startup/bl_operators/axismeld.py`; update `tests/python/axismeld_input_test.py`, `axismeld_input_blender.py`, `axismeld_hotbox_events.py`; update command mapping and architecture docs at their existing paths, README and phase report only as warranted; create `docs/compatibility/phase-2a-manual-test.md`.

**Inputs:** Native operators from Task 2, with properties exactly as specified there. Find actual module directory using `rg --files scripts -g '*commands.py'`; if it is nested below `axismeld/input` rather than the path above, change the existing files in place and record resolved paths, never create a duplicate module tree.
...
2. Implement metadata/validation and adapters. Forward actual invoke event semantics via native INVOKE_DEFAULT; do not hardcode Space on release, and no Python modal. View actions call native enum actions. Keep old scene/modeling checks and existing tool behavior. Hotbox target keymap is 3D View, not a duplicate in both Object/Mesh contexts.
3. Inspect generated Industry Compatible global maps for Space collision before connecting. Preserve global Frames/Screen; if a real overlapping global binding exists, report precise binding and resolve by a scoped active-context choice rather than silent deletion. RESERVED_KEYS no longer owns Space separately once hotbox owns it.
4. Add preference field to existing preset preferences and show accurate supported/adapted status; replace only now-obsolete 'hotbox not implemented' descriptions. Preserve any unrelated existing preferences, optional-file overrides and public-default/personal-config separation.
5. Change hotbox GUI tests to use actual product preset keymaps (remove Task 2's temporary test binding), including remap+disable with isolated settings. Assert default shortSpace toggles once, heldSpace directions, changing preset disables hotbox, other editors/active modal tools untouched and existing W/E/R remains covered by manipulator regression. Run `ctest -C Release --test-dir D:/source/AxisMeld-build -R axismeld --output-on-failure` and focused Python command/profile suites, record counts.
...
7. Install with `cmake --install D:/source/AxisMeld-build --config Release` only after tests; verify installed exe hash matches built exe and run isolated installed smoke using installed scripts, not source-script overrides. Do not launch over user instance or overwrite preferences. Do not claim visual smoothness or full Maya parity from synthetic tests.
