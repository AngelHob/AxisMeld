...
Cleanup outcome: the environment rejected the scratch-directory removal command before execution. No deletion was retried through another shell or API; the M1 scratch directory is retained. The implementation report and this ledger are already committed, and delivery is unaffected.
