...
| 入口 | 当前内容 |
|---|---|
| 开发源码 | [AngelHob/AxisMeld 的 axismeld/phase-2a](https://github.com/AngelHob/AxisMeld/tree/axismeld/phase-2a)；本次整理前 HEAD 为 `5b725820e51b0a277dbe929ca22d3a7063ea823c` |
...
| Rigging 工作区 | 默认资产新增 Rigging；Skeleton/Skin 两根；9 个真实 Blender 入口；保留 Maya 82 条正文和 33 个 Options | Maya 专属条目仍灰显；不能把原生 Weights 菜单的存在当作已实现 Maya Skin 命令 |
...
| A1 优先 | Skin：归一化与小权重清理 | 审计 Normalize Weights / Prune Small Weights；明确骨架、顶点范围、锁定、零权重与阈值；接入可证明的原生适配和独立 Options | 实际权重结果、无关顶点组保持、失败无修改、一次 Undo、参数入口和真实菜单路由有测试；差异明确可见 |
| A2 | Skin：影响骨与绑定管理 | Add/Remove/Remove Unused Influences，随后 Bind/Unbind；识别目标 Armature Modifier，区分保留权重、父级与变换 | 不误删其他用途组；多修改器、链接数据、无效选择和撤销有确定行为；不拿 Clear Parent 冒充解绑 |
...
A1 的第一步是语义和参数设计。Native `Normalize` 只把单个组最大值缩放为 1，不能映射为 Maya Normalize Weights；候选是 `Normalize All`，仍需审查范围和锁定。Maya Disable/Enable/Post 是持续模式，不是一次性归一化。Clean/Smooth/Limit Total 也不能直接宣称与 Maya 的 Prune/Smooth/最大影响数约束等价。
...
人工清单共 **308 项**，其中当前 296、已替代历史 12。47 项仅有历史“已测试”信息，不等于全通过；全表初始未测 261，剔除历史后的当前初始未测 249。新 RG-07～RG-14 均待测。每位网页访客有自己的记录，不能据维护者历史自动替他勾选。
...
新机器第一步按 [HANDOFF](HANDOFF.md) 核对分支和启动基线，再从 A1 开始；不要根据旧规格中的历史“尚未实现”重做已经交付的工作。
