...
- [x] 汇总未覆盖人工验收至 RG 后续稳定编号，保留全部原300条与用户状态；更新 CHANGELOG、公开目录、源码提交和预览包，推送 GitHub 并验证 Pages 与下载对应版本。
...
源码 `617986ac1841` 对应 `axismeld-2026.09.17-rigging-preview` 预览包；139 项运行资源、内嵌布局资产与实际解压启动均通过核对。默认资产采用单文件普通 Git 二进制存储，匿名拉取的 127672 字节与构建源 SHA256 一致。公开清单 308 项，新增 RG-07 至 RG-14 均为待人工测试。
